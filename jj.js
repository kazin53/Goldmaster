...
// ====================================
// SISTEMA DE LOBBY SIMPLIFICADO (NOVO)
// ====================================

let gameLobby = {
    isConnected: false,
    connectionType: null, // 'host' or 'guest'
    ...
    updateStatus: function() {
        ...
        if (!p2p) {
            status.innerHTML = 'Sistema P2P não inicializado';
            return;
        }
        
        // Determina se a conexão principal está ativa (apenas host e um guest)
        const isConnected = p2p.getConnectionCount() > 0;
        
        let html = `<strong>Status:</strong> ${isConnected ? 'Conectado' : 'Desconectado'}<br>`;
        html += `<strong>Tipo:</strong> ${p2p.isHost ? 'Host' : 'Guest'}<br>`;
        html += `<strong>Meu ID:</strong> ${p2p.localPlayerId}<br>`;
        html += `<strong>Conectados:</strong> ${p2p.getConnectionCount()}`;
        
        status.innerHTML = html;
        
        // Atualiza botão de iniciar jogo (só Host e se estiver conectado)
        const startBtn = document.getElementById('startGameBtn');
        if (startBtn) {
            startBtn.disabled = !p2p.isHost || !isConnected;
            startBtn.style.display = p2p.isHost ? 'block' : 'none';
        }
    },
    ...
    createRoom: async function() {
        try {
            // Inicializa a conexão como host
            await p2p.createRoom();
            this.connectionType = 'host';
            
            // Mostra o código para compartilhar
            const status = document.getElementById('connectionStatus');
            if (status) {
                status.innerHTML = `
                    <strong>Sala Criada!</strong><br>
                    <strong>Seu ID:</strong> ${p2p.localPlayerId}<br>
                    <strong>Compartilhe este código (Offer) com o Guest:</strong><br>
                    <code style="background: #333; padding: 5px; border-radius: 3px; display: block; margin: 10px 0; overflow-wrap: break-word;">
                        ${btoa(JSON.stringify(p2p.offer))}
                    </code>
                    <textarea id="guestAnswerInput" placeholder="Cole o ANSWER do Guest aqui" 
                           style="padding: 8px; margin: 5px 0; width: 100%; height: 80px;"></textarea>
                    <button onclick="gameLobby.processGuestAnswer()" 
                            style="padding: 8px; background: #4CAF50; color: white; border: none; border-radius: 3px; width: 100%; cursor: pointer;">
                        Conectar Guest
                    </button>
                `;
            }
            ...
            this.showError(`Erro ao criar sala: ${error.message || error}`);
        }
    },
    
    // Guest entra na sala
    joinRoom: async function() {
        const hostCodeInput = document.getElementById('hostIdInput');
        if (!hostCodeInput || !hostCodeInput.value.trim()) {
            this.showError('Cole o código (Offer) do Host!');
            return;
        }
        
        try {
            // Decodificar o offer do host
            const offerData = JSON.parse(atob(hostCodeInput.value.trim()));
            const answer = await p2p.handleOffer(offerData); // Usa handleOffer para obter o answer
            
            this.connectionType = 'guest';
            
            // Mostra o answer para enviar ao host
            const status = document.getElementById('connectionStatus');
            if (status) {
                status.innerHTML = `
                    <strong>Conectando ao Host...</strong><br>
                    <strong>Envie este código (Answer) para o Host:</strong><br>
                    <code style="background: #333; padding: 5px; border-radius: 3px; display: block; margin: 10px 0; overflow-wrap: break-word;">
                        ${btoa(JSON.stringify(answer))}
                    </code>
                    <p>Aguarde o Host processar o Answer e iniciar o jogo.</p>
                `;
            }
            
        } catch (error) {
            this.showError(`Erro ao entrar na sala: ${error.message || error}`);
        }
    },
    
    // Host processa answer do guest
    processGuestAnswer: async function() {
        const answerInput = document.getElementById('guestAnswerInput');
        if (!answerInput || !answerInput.value.trim()) {
            this.showError('Cole o ANSWER do Guest!');
            return;
        }
        
        try {
            const answerData = JSON.parse(atob(answerInput.value.trim()));
            await p2p.handleAnswer(answerData);
            
            this.updateStatus();
            
        } catch (error) {
            this.showError(`Erro ao processar ANSWER: ${error.message || error}`);
        }
    },
    
    // Iniciar jogo (somente Host)
    startGame: function() {
        if (!p2p || !p2p.isHost) return;
        
        gameLobby.hide();
        gameRunning = true;
        
        // Envia comando para o Guest iniciar o jogo também
        p2p.broadcast({ type: 'game_start' });
        console.log("Jogo iniciado pelo Host!");
    },
    
    // Receber comando de iniciar jogo (Guest)
    receiveGameStart: function() {
        gameLobby.hide();
        gameRunning = true;
        console.log("Jogo iniciado pelo Host remoto!");
    },
    
    showError: function(message) {
        const status = document.getElementById('connectionStatus');
        if (status) {
            status.innerHTML = `<strong style="color: #ff4444;">Erro:</strong> ${message}`;
        }
    }
};
...
// Enviar estado do jogador (para novos jogadores)
function sendPlayerState() {
    if (!p2p || p2p.getConnectionCount() === 0) return;
...
function sendPlayerUpdate() {
    ...
    const now = Date.now();
    if (now - lastNetworkUpdate < NETWORK_UPDATE_RATE) return;
...
function initMultiplayer() {
    ...
    p2p.onPlayerConnected = (peerId) => {
        console.log(`Jogador conectado: ${peerId}`);
        createRemotePlayer(peerId);
        gameLobby.updateStatus();
    ...
    p2p.onPlayerDisconnected = (peerId) => {
        console.log(`Jogador desconectado: ${peerId}`);
        removeRemotePlayer(peerId);
        gameLobby.updateStatus();
    };

    p2p.onDataReceived = (peerId, data) => {
    ...
    // Atualizar display do ID
    const idDisplay = document.getElementById('localPlayerIdDisplay');
    if (idDisplay) {
        idDisplay.textContent = localId;
    }
...
function handleNetworkData(peerId, data) {
    ...
    switch (data.type) {
        case 'player_state':
            ...
            // Atualizar estado inicial do jogador remoto
            if (remotePlayer) {
                remotePlayer.position.set(data.position.x, data.position.y, data.position.z);
                remotePlayer.rotation.y = data.rotation.y;
                // Lógica para atualizar nível/moedas do jogador remoto (se necessário na UI)
            }
            break;
        ...
        case 'player_update':
            if (remotePlayer) {
                ...
                remotePlayer.position.x += (data.x - remotePlayer.position.x) * 0.3;
                remotePlayer.position.y += (data.y - remotePlayer.position.y) * 0.3;
                remotePlayer.position.z += (data.z - remotePlayer.position.z) * 0.3;
...
function createRemotePlayer(peerId) {
    ...
    const color = PLAYER_COLORS[remotePlayers.size % PLAYER_COLORS.length];

    // Modelo simples do jogador remoto
    ...
    // Adicionar um identificador (para diferenciar os jogadores remotos)
    const labelGeometry = new THREE.PlaneGeometry(3, 0.5);
    const labelMaterial = new THREE.MeshBasicMaterial({ color: 0x000000, transparent: true, opacity: 0.7 });
    const label = new THREE.Mesh(labelGeometry, labelMaterial);
    label.position.y = 2.5;
    label.rotation.x = Math.PI / 2; // Para que o texto fique de frente para a câmera
    
    // Adiciona o texto (simplificado)
    ...
    context.fillText(peerId, 256, 80);
    
    const texture = new THREE.CanvasTexture(textCanvas);
    const textMaterial = new THREE.MeshBasicMaterial({ map: texture, transparent: true });
    const textMesh = new THREE.Mesh(new THREE.PlaneGeometry(2.5, 0.5), textMaterial);
    textMesh.position.set(0, 2.7, 0);
    textMesh.rotation.y = Math.PI; // Para que fique de frente para a câmera na rotação padrão
    remotePlayer.add(textMesh);


    scene.add(remotePlayer);
    remotePlayers.set(peerId, remotePlayer);
    
    console.log(`Jogador remoto criado: ${peerId}`);
}
...
// ====================================
// LOBBY UI (SUBSTITUIDO POR gameLobby)
// ====================================

function showLobbyUI() {
    // A função original foi substituída pela lógica do objeto gameLobby
    // Agora apenas configuramos os botões para chamar as funções do gameLobby
    
    const createBtn = document.getElementById('createRoomBtn');
    const joinBtn = document.getElementById('joinRoomBtn');
    const startBtn = document.getElementById('startGameBtn');
    
    if (createBtn) createBtn.onclick = () => { gameLobby.createRoom(); };
    if (joinBtn) joinBtn.onclick = () => { gameLobby.joinRoom(); };
    if (startBtn) startBtn.onclick = () => { gameLobby.startGame(); };

    // Chama o show do gameLobby após a inicialização do p2p
    // initMultiplayer() já faz isso.
}

function updateMultiplayerUI() {
    // A função original foi substituída por gameLobby.updateStatus()
    gameLobby.updateStatus();
}
...
// Configuração inicial
function init() {
    // Inicializar multiplayer PRIMEIRO
    initMultiplayer();
    ...
    // Inicializar UI (mas jogo não roda ainda)
    updateUI();

    // Iniciar loop do jogo (mas gameRunning = false até clicar "Iniciar Jogo")
    animate();
}
...
function depositCoins() {
    if (coinsCarried > 0) {
        ...
        // Notificar rede sobre o depósito
        if (p2p && p2p.getConnectionCount() > 0) {
            p2p.broadcast({ type: 'game_event', action: 'deposit' });
            sendPlayerState(); // Envia o novo estado de moedas
        }
    }
}
...
// ====================================
// FUNÇÕES GLOBAIS PARA O LOBBY (adicionar no FINAL do arquivo)
// ====================================

// Funções globais foram substituídas pelas funções do objeto gameLobby,
// que agora são acessíveis globalmente através da variável `gameLobby`.

/*
window.handleGuestOffer = async (offerData) => { ... }
window.handleHostAnswer = async (answerData) => { ... }
*/
